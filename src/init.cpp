...
switch( the_signal )
    ...
    case CTRL_SHUTDOWN_EVENT:   // I think we should do something here too?   
        ...
        // so we handle all 5 of these cases!
        // I tested all 5 andthey all work! Ron
...
#define PROCESS_DEP_ENABLE 0x00000001
  ...
  //    // we failed!
    ...
    // SetConsoleCtrlHandler is very much similar to signal(), i.e.
    ...
    // for linux, I see, http://linux.die.net/man/1/erlsrv

    // the Window's signal is as below
    ...
    //               __GNUC_MINOR__ * 100 + 
    //               __GNUC_PATCHLEVEL__
    printf("Using Boost GCC full version %d.d patch level %d\n",        //AAIIpp
            // __GNUC__,
            BOOST_GCC / 10000,
    ...
    if (pindexBest && pindexBest != pindexRescan)
    {
        ...
        BOOST_FOREACH(const PAIRTYPE(CBitcoinAddress, string)& item, pwalletMain->mapAddressBook)
        {
            const CBitcoinAddress
                & addressFound = item.first;

            const string
                & strNameFound = item.second;

            if (strNameFound == strAccountToMatch)
            {                
                if( fPrintToConsole )
                {
                    (void)printf(
        ...
        nStart = GetTimeMillis();
#ifdef _MSC_VER
        void( pwalletMain->ScanForWalletTransactions(pindexRescan, true, pindexBest->nHeight - pindexRescan->nHeight) );
        // we are intentionally ignoring the result of this function
        // which is telling us whether or not we have had any updates to our wallet
        // Is this the intent??

        // And should it only rescan backto (or is it starting from) the "birthday" of the wallet,
        // if that is known??
#else
